...
/*  The UserDetails interface has 7 methods that needs to be implemented.
    ...
    For the framework the difference between a role and an authority is minimal and they are stored together. To tell
    them from one another we prefix a role with ROLE_. If there is no prefix, we're dealing with an authority.
